/**
 * fetchKanji.mjs
 * Scarica i dettagli di tutti i kanji N1–N5 da KanjiAPI.dev
 * e salva il risultato in src/data/kanjiDataFull.json
 *
 * Usage: node scripts/fetchKanji.mjs
 */
  ...
  try {
    ...
    if (res.status === 429) {
      if (retries < MAX_RETRIES) {
        console.warn(`  ⚠ Rate limit, attendo ${RETRY_DELAY_MS}ms…`)
...
// ── Cache per riprendere un download interrotto ───────────────────────────────
function loadCache() {
...
async function main() {
  ...
  if (cachedCount > 0) {
    console.log(`📦 Cache trovata: ${cachedCount} kanji già scaricati. Riprendo da dove mi ero fermato…\n`)
  }
  ...
  for (const [levelStr, chars] of Object.entries(KANJI_BY_LEVEL)) {
    const level = parseInt(levelStr)
    const levelName = LEVEL_NAMES[level]
    const todo = chars.filter((c) => !cache[c])
    console.log(`\n── ${levelName} (${chars.length} totali, ${todo.length} da scaricare) ──`)
    ...
    for (let i = 0; i < chars.length; i++) {
      ...
      try {
        ...
        // Salva cache ogni 50 kanji per non perdere progressi
        if (fetched % 50 === 0) {
          saveCache(cache)
          const total = Object.values(KANJI_BY_LEVEL).flat().length
          const done  = Object.keys(cache).length
          console.log(`  ✓ ${done}/${total} kanji scaricati`)
        }

        await delay(DELAY_MS)
      } catch (err) {
        console.error(`  ✗ Errore per "${char}": ${err.message}`)
        errors++
      }
    }
  }
  ...
  // Ordina: prima per livello (5→1), poi nell'ordine originale
  const orderedChars = Object.values(KANJI_BY_LEVEL).flat()
  const ordered = orderedChars
    .map((c) => cache[c])
    .filter(Boolean)

  writeFileSync(OUTPUT_PATH, JSON.stringify(ordered, null, 2), 'utf-8')

  console.log(`\n✅ Completato!`)
  console.log(`   Kanji salvati : ${ordered.length}`)
  console.log(`   Errori        : ${errors}`)
  console.log(`   File          : src/data/kanjiDataFull.json`)

  if (errors > 0) {
    console.log(`\n⚠ Ci sono stati ${errors} errori. Riesegui lo script per riprovare i kanji mancanti.`)
  }
}
